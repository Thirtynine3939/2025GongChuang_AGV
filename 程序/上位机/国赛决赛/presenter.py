...
from matter_params import Matter
...
class AppPresenter(QObject):
    ...
    def __init__(self, parent=None):
        ...
        # 初始化各层组件
        self.matter1 = Matter(jawheight=47.0, matterheight=57.0, grabheight=0) # 物料1
    ...
    def _init_ui_connect_debug(self):
        ...
        # 脉冲正解按钮
        self.view.btn_clk2pos.clicked.connect(self._clk2pos)

        # 运动至全局托盘放置点按钮
        self.view.btn_globaltrayP.clicked.connect(self._move_to_global_trayP)

        # 运动至全局托盘悬浮点按钮
        self.view.btn_globaltrayH.clicked.connect(self._move_to_global_trayH)

        # 运动至全局托盘抓取点按钮
        self.view.btn_globaltrayG.clicked.connect(self._move_to_global_trayG)

        # 新物料跟踪起点
        self.view.btn_newtrackbegin.clicked.connect(self._move_to_newtrack)

    # 决赛
    @Slot()
    def _move_to_newtrack(self):
        ZDT.Emm_V5_Pos_Control(4,1,1200,120,MP.J4clk_normal,1,0)
        time.sleep(0.005)
        self.arm.MoveP_AccDec(0, 289, 230, MP.VelAcc.ToTrackBegin)
    ...
    def _read_joint_clkPos(self):
        ...
        clk2 = -ZDT.Motor_CLK[1] # 电机产品问题，57电机脉冲方向相反
    ...
    def _clk2pos(self):
        """脉冲正解"""
        J1 = int(self.view.entryR_j1Clk.text())
        J2 = int(self.view.entryR_j2Clk.text())
        J3 = int(self.view.entryR_j3Clk.text())

        x, y, z = self.arm.pos_cal_forward(J1, J2, J3)  # 使用机械臂正解计算位置
        self.view.entryR_globalX.setText(f"{x:.1f}")
        self.view.entryR_globalY.setText(f"{y:.1f}")
        self.view.entryR_globalZ.setText(f"{z:.1f}")

    @Slot()
    def _move_to_global_trayP(self):
        """运动至全局托盘放置点"""
        num = int(self.view.cBox_MPG_num.currentText())
        vel = int(self.view.entry_MPG_vel.text())
        acc = int(self.view.entry_MPG_acc.text())

        self.arm.MoveP_AccDec_global(self.matter1.tray_place, 
                                     MP.VelAcc.AUTO, 
                                     [num, vel, acc])
        self.add_log(f"已运动至全局托盘放置点: {self.matter1.tray_place}, 基准电机号={num}, 速度={vel}, 加速度={acc}")

    @Slot()
    def _move_to_global_trayH(self):
        """运动至全局托盘悬停点"""
        # num = int(self.view.cBox_MPG_num.currentText())
        # vel = int(self.view.entry_MPG_vel.text())
        # acc = int(self.view.entry_MPG_acc.text())

        self.arm.MoveP_AccDec_global(self.matter1.tray_hover, 
                                     MP.VelAcc.ToTrayHover_Normal)
        self.add_log(f"已运动至全局托盘悬停点")

    @Slot()
    def _move_to_global_trayG(self):
        """运动至全局托盘抓取点"""
        num = int(self.view.cBox_MPG_num.currentText())
        vel = int(self.view.entry_MPG_vel.text())
        acc = int(self.view.entry_MPG_acc.text())

        self.arm.MoveP_AccDec_global(self.matter1.tray_grab, 
                                     MP.VelAcc.AUTO, 
                                     [num, vel, acc])
        self.add_log(f"已运动至全局托盘抓取点: {self.matter1.tray_grab}, 基准电机号={num}, 速度={vel}, 加速度={acc}")
